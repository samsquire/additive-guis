...
def layout_page(predicates, widgets, classes):
    ...
    if status == cp_model.FEASIBLE or status == cp_model.OPTIMAL:
        ...
        yield "<div class=\"container {}\">".format(classes)
        for row_id, columns in grouped:
            ...
            for key, cell in cells:
                ...
                yield "<div class=\"col col-md-{}\">".format(sizes.get(key,siblings.get("hasDefaultSize")))
                render_data = widgets.get(key, {})
                if "html" in render_data:
                    yield render_data["html"]
                elif "predicates" in render_data:
                    yield from layout_page(render_data["predicates"], widgets, render_data.get("classes", ""))
                else:
                    yield key
                yield "</div>"
...
# yield "<head><link href=\"https://getbootstrap.com/docs/4.0/examples/blog/blog.css\" rel=\"stylesheet\"></head>"
...
def render():
    def generate():
        ...
        for container in all_predicates:
            yield from layout_page(container, widgets, "")
